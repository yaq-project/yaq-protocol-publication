...
%DIF DEL paper-as-submitted.tex   Sun Feb  5 20:06:44 2023
%DIF ADD paper.tex                Thu Mar 16 10:56:22 2023
...
%DIFDELCMD <   University of Wisconsin--Madison
%DIFDELCMD < }
%DIFDELCMD < %%%
...
\DIFaddend \email{blaise.thompson@wisc.edu}
...
\DIFaddbegin \DIFadd{More work needs to be done to catalog, compare, and contrast the large number of open-source projects that now exist.
}\DIFaddend Most small custom research instrumentation continues to rely on monolithic software which has hard-coded interface support for each particular connected device.
These monolithic applications tend to be inflexible and difficult to develop.

We have created a new network-based communication standard for scientific instrumentation, \yaq{} \DIFaddbegin \DIFadd{(Yet Another acQuisition)\footnote{see Supplementary Material for a full description of the name \yaq{}}}\DIFaddend .
...
\DIFdelbegin \DIFdel{Our primary goal has been to make an interfacewhich simplifies orchestration softwaredevelopment as much as possible}\DIFdelend \DIFaddbegin \DIFadd{Via the \yaq{} interface, we provide easy-to-use hardware support for scientists to use with their choice of experiment orchestration software}\DIFaddend .
...
Each \DIFdelbegin \DIFdel{individual communication between client and daemon involves one message being requested by the client and the response returned from the daemon.
Each }\DIFdelend daemon supports a collection of messages for its unique functionality, called the ``protocol''.
...
\DIFaddbegin \DIFadd{Each individual communication between client and daemon involves one message being requested by the client and the response returned from the daemon.
...
Users may choose to implement access control systems, but we do not foresee building access control into the }\yaq{} \DIFadd{protocol.
This is similar to how access control works in practice at some large facilities.
}

\DIFaddend \yaq{} introduces a concept called ``traits'' which are collections of related messages that are shared among multiple protocols.
...
\DIFaddbegin \DIFadd{Traits are compositional: a given protocol may implement several traits at once.
For example, a protocol representing a monochromator with several gratings might implement both ``}\texttt{\DIFadd{has-position}}\DIFadd{`` and ``}\texttt{\DIFadd{has-turret}}\DIFadd{``, where the latter defines special messages for choosing which grating is used.
}\DIFaddend Importantly, specific protocols can also implement arbitrary additional messages that are not defined by any trait.

\DIFaddbegin \DIFadd{We have carefully defined \yaq{} traits to maximize interoperability while not excluding hardware with unusual features.
...
\DIFadd{We now describe how the \yaq{} architecture addresses the three hardware interface barriers described at the beginning of this section.
}\DIFaddend The first hardware interface barrier: Multiple incompatible interfaces are used to communicate with each component of the system. % consider making this sentence (and the next paragraph starts) subsection headings
...
The third hardware interface barrier: Some hardware interfaces are blocking \DIFdelbegin \DIFdel{.
Monolithic }\DIFdelend \DIFaddbegin \DIFadd{or slow.
Experimental orchestration often means doing several things at once, }\textit{\DIFadd{e.g.}} \DIFadd{simultaneously moving several motors.
To accomplish this task performantly, monolithic }\DIFaddend orchestration software often necessitates separate threads for each component hardware interface, a fragile pattern in which small mistakes become both critical and elusive errors.
\DIFdelbegin \DIFdel{In }\DIFdelend \DIFaddbegin \DIFadd{When writing orchestration software using }\DIFaddend \yaq{}, \DIFdelbegin \DIFdel{each daemon is only responsible for a single hardware interface.
%DIF < Each \yaq{} daemon runs as a separate process (possibly not even on the same machine) and thus many of the hard concurrency problems with communicating with hardware are mitigated.
It is expected that each message call over }\DIFdelend \DIFaddbegin \DIFadd{slow hardware interfaces are replaced by fast Avro-RPC.
In our experience, }\DIFaddend the \yaq{} interface \DIFdelbegin \DIFdel{will return rapidly, ensuring that client applications are not blocked for extended periods of time.
This principle applies even when the message starts an action that might take several seconds to complete}\DIFdelend \DIFaddbegin \DIFadd{is responsive enough to give good performace when orchestrating tens of motors using a singly-threaded client.
There is a limit where using TCP becomes a bottleneck for large resonses}\DIFaddend , such as \DIFdelbegin \DIFdel{homing a motor or initiating a measurement for a sensor.
...
In order to know how long to wait, the ``is-daemon`` trait provides a message called ``is\_busy'', which returns "true" while the long running action is not complete, and "false" once it is finished.
Additionally, multiple clients can communicate with the same daemon simultaneously.
A complex instrument may involve multiple operators watching sensor data in real time, while one program is orchestrating the hardware and recording the data}\DIFdelend \DIFaddbegin \DIFadd{those from cameras or other high throughput sensors.
In our experience, this limit is approximately 1 megapixel.
For larger sensors, a pure \yaq{} approach may not be appropriate.
There is a longer discussion of timing and order-of-operations control details using \yaq{} in the Supplementary Material}\DIFaddend .
...
At the bottom, we \DIFdelbegin \DIFdel{show several features associated with }\DIFdelend \DIFaddbegin \DIFadd{graphically represent }\DIFaddend the integration between \yaq{} and the Bluesky project\cite{AllanDanielB2019a}.
...
\DIFdelbegin \DIFdel{Shown in Figure \ref{fig:foundation}, we see one such GUI for parameterizing a Bluesky procedure and a representation of the resultant data}\DIFdelend \DIFaddbegin \DIFadd{Bluesky has no built-in hardware interface support, instead relying on packages to create hardware-interfaces that are compatible with their Hardware Protocol defintions}\DIFaddend .
\texttt{yaqc-bluesky}\footnote{\texttt{yaqc-bluesky} available on PyPI at https://pypi.org/project/yaqc-bluesky/} \DIFdelbegin \DIFdel{provides a bridge to the Blueskyecosystem}\DIFdelend \DIFaddbegin \DIFadd{is a specialized client which adapts any }\yaq{} \DIFadd{protocol into Bluesky}\DIFaddend .
Similar translation layers could be built for a variety of orchestration \DIFdelbegin \DIFdel{layers }\DIFdelend \DIFaddbegin \DIFadd{software }\DIFaddend such as PyMoDAQ\cite{WeberSebastien2021a}, \DIFdelbegin \DIFdel{Instrumental\mbox{%DIFAUXCMD
\cite{Bogdanowicz2022}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{PyMeasure\mbox{%DIFAUXCMD
\cite{pymeasure}}\hskip0pt%DIFAUXCMD
}\DIFaddend , or TRSpectrometer\cite{trspectrometer}.
...
\DIFaddbegin \DIFadd{Separability, portability, and distributed development are advantages common to many open-source hardware interface projects.
}\DIFaddend Third, when developing trait-compliant protocols, it becomes easy to design and fully test your hardware interface.
...
Software \DIFdelbegin \DIFdel{documentation is famously difficult and thankless work. }\DIFdelend \DIFaddbegin \DIFadd{built by scientists often has incomplete and inadequate documentation. \mbox{%DIFAUXCMD
\cite{SegalJudith2007a}
}\hskip0pt%DIFAUXCMD
}\DIFaddend \yaq{} attempts to automate daemon documentation as much as possible.
...
\DIFaddbegin \DIFadd{We have designed \yaq{} to be easier to deploy and maintain when compared to EPICS and TANGO.
...
We provide tooling (}\texttt{\DIFadd{yaqd-control}}\DIFadd{) which makes it easy to configure, list, and manage daemons as background services on Windows, MacOS, or Linux (See SI for more details).
}

\DIFaddend \yaq{} is open source software.
...
\DIFaddbegin \DIFadd{More detailed examples of orchestration using \yaq{}, including scripts and graphical user interfaces.
Detailed description of }\texttt{\DIFadd{yaqd-control}}\DIFadd{.
...
We would also like to acknowledge the developers of the broader \DIFdelbegin \DIFdel{open source }\DIFdelend \DIFaddbegin \DIFadd{open-source }\DIFaddend software community upon-which this project rests.
