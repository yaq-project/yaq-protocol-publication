...
%DIF ADD paper.tex                Sun Feb  5 19:36:06 2023
...
\DIFaddbegin \DIFadd{If multiple messages are sent simultaneously, the daemon processes one message at a time.
The protocol is preemptive: in the case of a conflict, the daemon will obey the last message to arrive.
}\DIFaddend Each daemon supports a collection of messages for its unique functionality, called the ``protocol''.
...
Our website also contains written and video tutorials on \yaq{} usage and development\DIFaddbegin \DIFadd{.
}

\DIFadd{We have designed }\yaq{} \DIFadd{to be easier to deploy and maintain when compared to EPICS and TANGO.
...
We provide tooling (}\texttt{\DIFadd{yaqd-control}}\DIFadd{) which makes it easy to configure, list, and manage daemons as background services on Windows, MacOS, or Linux (See SI for more details)}\DIFaddend .
